...
- 最佳实战
	- 配置 类组件之间**无依赖关系**用Lite模式(ProxyBeanMethods=false)加速容器启动过程，减少判断
	- 配置 类组件之间**有依赖关系**，方法会被调用得到之前单实例组件，用Full模式(ProxyBeanMethods=true）（默认）
...
//5、获取组件
//通过imprt注解方式注册的组件，获取的名字默认是全类名com.atguigu.boot.bean.User，ch.qos.logback.core.db.DBHelper
String[] beanNamesForType = run.getBeanNamesForType(User.class);

for (String s : beanNamesForType) {
    System.out.println(s);//com.atguigu.boot.bean.User    user01

}

DBHelper bean1 = run.getBean(DBHelper.class);
System.out.println(bean1);  //ch.qos.logback.core.db.DBHelper
```
...
- 每个自动配置类按照条件进行生效，默认都会绑定配置文件指定的值。（值从xxxxProperties里面读取，xxxProperties和配置文件进行了绑定）
- 生效的配置类就会给容器中装配很多组件
...
第一步：spring boot已经管理Lombok。引入依赖：
...
第二步：IDEA中File->Settings->Plugins，搜索安装Lombok插件。

```java
@NoArgsConstructor		//帮忙生成无参构造器
//@AllArgsConstructor  //帮忙生成有参构造器
@Data //帮助生成属性的getter和setter方法
@ToString //帮助生成toString方法
@EqualsAndHashCode//帮忙生成equals和hashcode方法
public class User {
...
```yaml
由于matchIfMissing默认是false，在hiddenHttpMethodFilter类中没有声明由于matchIfMissing默认是false
是不开启的，需要手动开启
spring:
  mvc:
    hiddenmethod:
...
- SpringBoot自动配置了默认 的 RequestMappingHandlerMapping，用来解析@RequestMapping或者@Get/PostMapping等注解
